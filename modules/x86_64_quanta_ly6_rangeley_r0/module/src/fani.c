...
#include <x86_64_quanta_ly6_rangeley_r0/x86_64_quanta_ly6_rangeley_r0_gpio_table.h>
#include <onlp/platformi/fani.h>

#include "x86_64_quanta_ly6_rangeley_r0_int.h"
#include "x86_64_quanta_ly6_rangeley_r0_log.h"
...
sys_fan_info_get__(onlp_fan_info_t* info, int id)
{
    ...
    if(info->rpm <= X86_64_QUANTA_LY6_RANGELEY_R0_CONFIG_SYSFAN_RPM_FAILURE_THRESHOLD) {
        info->status |= ONLP_FAN_STATUS_FAILED;
    }
    ...
    if(info->status & ONLP_FAN_STATUS_F2B) {
        info->percentage = (int) ((double) info->rpm * (double)100 / (double)X86_64_QUANTA_LY6_RANGELEY_R0_CONFIG_SYSFAN_F2B_RPM_MAX);
    }
    if(info->status & ONLP_FAN_STATUS_B2F) {
        info->percentage = (int) ((double) info->rpm * (double)100 / (double)X86_64_QUANTA_LY6_RANGELEY_R0_CONFIG_SYSFAN_B2F_RPM_MAX);
    }
